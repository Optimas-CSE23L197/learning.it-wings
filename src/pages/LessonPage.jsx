...
export default function LessonPage() {
    ...
    return (
        <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10 py-8 sm:py-10">
            {/* ===== Header ===== */}
            <div className="mb-10">
            ...
            {/* ===== Simple Paragraph Lesson ===== */}
            {lesson.paragraphs && (
                <section className="space-y-6 text-[var(--gray-light)]">
            ...
            {/* ===== Advanced Sectioned Lesson ===== */}
            {lesson.sections && (
                <section className="space-y-10 text-[var(--gray-light)]">
                    {lesson.sections.map((sec, i) => (
                        <div key={i} className="space-y-4">
                            ...
                            {/* Single Example */}
                            {sec.example && (
                                <pre className="bg-[var(--blue-medium)] p-4 rounded-lg overflow-x-auto text-sm text-[var(--white)]">
                                    <code>{sec.example}</code>
                                </pre>
                            )}

                            {/* Multiple Examples */}
                            {sec.examples && sec.examples.map((ex, idx) => (
                                <pre
                                    key={idx}
                                    className="bg-[var(--blue-medium)] p-4 rounded-lg overflow-x-auto text-sm text-[var(--white)]"
                                >
                                    <code>{ex}</code>
                                </pre>
                            ))}
